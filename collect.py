...
def collect_data():
    ...
    root = tk.Tk()
    root.title("Sign Language Video Data Collection")
    ...
    word_frame = tk.LabelFrame(root, text=" select word to collect  ", padx=10, pady=10)
    ...
    current_word_label = tk.Label(status_frame, text="Current Word: None")
    current_word_label.pack(fill=tk.X, pady=5)
    
    progress_label = tk.Label(status_frame, text="Progress: 0/0")
    progress_label.pack(fill=tk.X, pady=5)
    
    total_videos_label = tk.Label(status_frame, text="Total Videos: 0")
    total_videos_label.pack(fill=tk.X, pady=5)
    
    hands_detected_label = tk.Label(status_frame, text="Hands Detected: 0")
    hands_detected_label.pack(fill=tk.X, pady=5)
    
    recording_status = tk.Label(status_frame, text="Not Recording", fg="black")
    ...
    landmarks_check = tk.Checkbutton(options_frame, text="Show Hand Landmarks", variable=show_landmarks)
    ...
    tk.Label(duration_frame, text="Video Duration (seconds):").pack(side=tk.LEFT)
    ...
    tk.Label(samples_frame, text="Number of Videos to Add:").pack(side=tk.LEFT)
    ...
    def select_word(word):
        ...
        sample_count.set(0)
        current_word_label.config(text=f"Current Word: {word.replace('_', ' ').title()}")
    ...
    def update_hands_label(count):
        hands_detected_label.config(text=f"Hands Detected: {count}")
    
    # Function to update recording status
    def update_recording_status(is_recording, progress=0):
        if is_recording:
            recording_status.config(text=f"Recording... {progress}%", fg="red")
            preview_progress['value'] = progress
        else:
            recording_status.config(text="Not Recording", fg="black")
...
def camera_loop(cap, current_word, sample_count, max_samples, update_progress_callback, 
    ...
    while True:
        ...
        if word:
            if recording.get():
                status_text = f"Word: {word} | Recording: {count+1}/{max_count}"
            else:
                status_text = f"Word: {word} | Progress: {count}/{max_count} | Press 'v' to record"
        else:
            status_text = "Select a word from the menu"
        ...
        if recording.get():
            if video_writer is not None:
                ...
                if elapsed_time >= video_duration.get():
                    ...
                    cv2.putText(display_frame, "Video Saved!", (w//2 - 100, h//2), 
